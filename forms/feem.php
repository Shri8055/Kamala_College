...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (!empty($_POST['pay_fee_id']) || !empty($_POST['non_fee_id'])) {
    // Paying
    $pay_scope_input = $_POST['pay_scope'] ?? [];
    if (!empty($_POST['pay_fee_id'])) {
      foreach ($_POST['pay_fee_id'] as $i => $fee_id) {
        ...
        $scope = (isset($pay_scope_input[$fee_id]) && $pay_scope_input[$fee_id] === 'university') ? 'university' : 'college';

        $stmt = $conn->prepare("UPDATE feestru SET sh_nm=?, fl_nm=?, amount=?, fee_scope=? WHERE fee_id=? AND term_id=?");
        $stmt->bind_param("ssdsii", $sh_nm, $fl_nm, $amount, $scope, $fee_id, $term_id);
        $stmt->execute();
      }
    }
    // Non-Paying
    $non_scope_input = $_POST['non_scope'] ?? [];
    if (!empty($_POST['non_fee_id'])) {
      foreach ($_POST['non_fee_id'] as $i => $fee_id) {
        ...
        $scope = (isset($non_scope_input[$fee_id]) && $non_scope_input[$fee_id] === 'university') ? 'university' : 'college';
  ...
  if (!empty($_POST['pay_sh_nm'])) {
    foreach ($_POST['pay_sh_nm'] as $index => $sh_nm) {
      ...
      if ($sh_nm !== '') {
        ...
        $scope = (isset($_POST['pay_scope'][$index]) && $_POST['pay_scope'][$index] == 'university') ? 'university' : 'college';
        $stmt = $conn->prepare("INSERT INTO feestru (term_id, type, sh_nm, fl_nm, amount, fee_scope) 
                        VALUES (?, 'paying', ?, ?, ?, ?)");
        $stmt->bind_param("issds", $term_id, $sh_nm, $fl_nm, $amount, $scope);
        $stmt->execute();
      }
    }
  }
  if (!empty($_POST['non_sh_nm'])) {
    foreach ($_POST['non_sh_nm'] as $index => $sh_nm) {
      ...
      if ($sh_nm !== '') {
        ...
        $scope = (isset($_POST['non_scope'][$index]) && $_POST['non_scope'][$index] == 'university') ? 'university' : 'college';
        $stmt = $conn->prepare("INSERT INTO feestru (term_id, type, sh_nm, fl_nm, amount, fee_scope) 
                        VALUES (?, 'non_paying', ?, ?, ?, ?)");
        $stmt->bind_param("issds", $term_id, $sh_nm, $fl_nm, $amount, $scope);
        $stmt->execute();
...
$res = $conn->query("SELECT fee_id, term_id, type, fee_scope, sh_nm, fl_nm, amount FROM feestru WHERE fl_nm <> 'Total' ORDER BY fee_id ASC");
...
<head>
  ...
  <style>
    ...
    .card-details {
      ...
      padding: 10px;
      background: #f9f9f9;
      width: 95%;           /* or 100% of parent */
      max-width: 1600px;    /* optional cap */
      box-sizing: border-box;
    }

    .card-details table {
      width: 100%;          /* make tables fill parent */
      border-collapse: collapse;
    }
    .card-details > form > div {
      display: flex;
      gap: 40px;
    }

    .card-details > form > div > div {
      flex: 1; /* both Paying and Non-Paying take equal space */
    }
  </style>
</head>
<body>
  <form method="POST">
    <h3 style="text-align:center; padding-bottom: 5px;">Fee Structure Entry</h3>
    ...
    <div style="display:flex;">
      ...
      <table id="payingTable">
        ...
        <tr>
          ...
          <td style="text-align:center;">
            <input type="checkbox" name="pay_scope[]" value="university">
          </td>
        </tr>
      </table>
      ...
      <table id="nonPayingTable">
        ...
        <tr>
          ...
          <td style="text-align:center;">
            <input type="checkbox" name="non_scope[]" value="university">
          </td>
        </tr>
      </table>
          ...
          <div style="flex:1;">
            ...
            <table border="1" cellpadding="4" cellspacing="0">
              ...
              <?php foreach ($data['paying'] as $row): ?>
                <tr>
                  <td><input type="text" name="pay_sh_nm[]" value="<?= htmlspecialchars($row['sh_nm']) ?>"></td>
                  <td><input type="text" name="pay_fl_nm[]" value="<?= htmlspecialchars($row['fl_nm']) ?>"></td>
                  <td><input type="number" step="0.01" style="text-align: right;" class="fee-input pay-fee" name="pay_amount[]" value="<?= $row['amount'] ?>"></td>
                  <td style="text-align:center;">
                    <input type="checkbox" name="pay_scope[<?= $row['fee_id'] ?>]" value="university" <?= ($row['fee_scope'] == 'university' ? 'checked' : '') ?>>
                  </td>
                  <input type="hidden" name="pay_fee_id[]" value="<?= $row['fee_id'] ?>">
                </tr>
              <?php endforeach; ?>
              <tfoot>
                <tr>
                  <td colspan="2" style="text-align:right;font-weight:bold;">Total</td>
                  <td id="payingTotal-<?= $cid ?>" style="text-align:right;font-weight:bold;">0.00</td>
                  <td></td>
                </tr>
              </tfoot>
          ...
          <div style="flex:1;">
            ...
            <table border="1" cellpadding="4" cellspacing="0" style="width:100%;">
              ...
              <?php foreach ($data['non_paying'] as $row): ?>
                <tr>
                  <td><input type="text" name="non_sh_nm[]" value="<?= htmlspecialchars($row['sh_nm']) ?>"></td>
                  <td><input type="text" name="non_fl_nm[]" value="<?= htmlspecialchars($row['fl_nm']) ?>"></td>
                  <td><input type="number" step="0.01" style="text-align: right;" class="fee-input non-fee" name="non_amount[]" value="<?= $row['amount'] ?>"></td>
                  <td style="text-align:center;">
                    <input type="checkbox" name="non_scope[<?= $row['fee_id'] ?>]" value="university" <?= ($row['fee_scope'] == 'university' ? 'checked' : '') ?>>
                  </td>
                  <input type="hidden" name="non_fee_id[]" value="<?= $row['fee_id'] ?>">
                </tr>
              <?php endforeach; ?>
              <tfoot>
                <tr>
                  <td colspan="2" style="text-align:right;font-weight:bold;">Total</td>
                  <td id="nonPayingTotal-<?= $cid ?>" style="text-align:right;font-weight:bold;">0.00</td>
                  <td></td>
                </tr>
              </tfoot>
            </table>
  ...
  <script>
    function addRow(tableId, prefix) {
      ...
      row.insertCell(2).innerHTML = `<input type="number" step="0.01" class="${prefix === 'pay' ? 'pay-fee' : 'non-fee'}" name="${prefix}_amount[]">`;
      row.insertCell(3).innerHTML = `<input type="checkbox" name="${prefix}_scope[]" value="university">`;
    }
    document.getElementById('copyData').addEventListener('change', function() {
      if (this.checked) {
        ...
        // Reset Non-Paying table headers
        nonTable.innerHTML = `
            <tr><th colspan="4">Non-Paying</th></tr>
            <tr><th>Short Name</th><th>Full Name</th><th>Amount</th><th>University Fee?</th></tr>
        `;
        payRows.forEach((row, i) => {
          if (i > 1) { // skip headers
            ...
            let checked = row.querySelector(`input[name='pay_scope[]']`)?.checked ? 'checked' : '';
            ...
            newRow.insertCell(3).innerHTML = `<input type="checkbox" name="non_scope[]" value="university" ${checked}>`;
          }
        });
      }
    });
  ...
  <script>
    function calculateTotals() {
      document.querySelectorAll('.card-details').forEach(card => {
        let paySum = 0;
        card.querySelectorAll('.pay-fee').forEach(input => {
          paySum += parseFloat(input.value) || 0;
        });
        let payTotalEl = card.querySelector('[id^="payingTotal"]');
        if (payTotalEl) payTotalEl.textContent = paySum.toFixed(2);

        let nonSum = 0;
        card.querySelectorAll('.non-fee').forEach(input => {
          nonSum += parseFloat(input.value) || 0;
        });
        let nonTotalEl = card.querySelector('[id^="nonPayingTotal"]');
        if (nonTotalEl) nonTotalEl.textContent = nonSum.toFixed(2);
      });
    }

    document.addEventListener('input', function(e) {
      if (e.target.classList.contains('pay-fee') || e.target.classList.contains('non-fee')) {
        calculateTotals();
      }
    });
    // Run on load
