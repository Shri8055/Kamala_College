...
// Split class (Example: "BACHELOR OF COMMERCE - F.Y.")
$clsFull = preg_replace('/\s+/', ' ', $clsFull); // collapse spaces
$parts = array_map('trim', explode('-', $clsFull));
$clsLeft = $parts[0] ?? '';
$termRight = $parts[1] ?? '';

// Find term_id and cls_id
$stmt = $conn->prepare("SELECT cls_id, term_id FROM feecls WHERE cls_ful_nm=? AND term_title=? LIMIT 1");
